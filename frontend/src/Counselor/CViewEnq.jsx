...
function CViewEnq() {
    ...
    const getEnq = async ()=>{
      try {
        ...
        const res2 = await axios.get(`http://localhost:3000/api/user/${localStorage.getItem('Counselor')}`)
          if(res.data.msg == "Success" && res2.data.msg == "Success"){
            console.log(res.data.enq);
            var enquries = res.data.enq.filter((a)=>{
              return a.assignto ? a.assignto._id == localStorage.getItem('Counselor') : a.center == res2.data.user.centers; // yaha ":" k baad agr TRUE hota h to mtlb jo kisi ko assign nhi h to vo DIKHEGA (sbko) & localStorage.getItem('Counselor') me current user ki id aati h jo login h CURENTLY
            });
            console.log(enquries);
            setEnq(enquries);  //specific center ki enq filter ho rhi h
            console.log(res.data.enq);
          }
      } 
      catch (error) {
